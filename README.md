...
- **[ffmpeg](https://ffmpeg.org/)** on your `PATH` (or **`FFMPEG_PATH`** in `.env`) only if you use **`transcode:`** HLS URLs, or the legacy **`TWITCH_PLAYBACK=hls`** mode. **Not required** for the default Twitch playback (see [HLS and transcoding](#hls-and-transcoding))
- **[Streamlink](https://streamlink.github.io/)** on your `PATH` (or set **`STREAMLINK_PATH`**) — **optional**. The default Twitch playback (**proxy** mode) works without it by calling Twitch's API directly. Streamlink is only used as a fallback if the direct API fails, or if you force **`TWITCH_PLAYBACK=hls`** (legacy ffmpeg transcode).
...
| `TWITCH_PLAYBACK` | No | Twitch playback mode. **`proxy`** (default): same-origin HLS via a **pass-through proxy** — resolves CDN URLs via Twitch's GQL + usher API directly (**no Streamlink or ffmpeg install needed**), and each tile's quality adapts to its pixel size (small tiles get 360p/480p, large tiles get 720p60) so 15+ simultaneous streams stay smooth. If Streamlink is installed, it's used as a fallback when the direct API fails. **`hls`**: legacy **streamlink + ffmpeg** re-segment (high CPU, one transcode per tile, requires both tools). **`iframe`**: official Twitch embed (no server CPU, but browsers cap ~16 simultaneous WebGL contexts, so 15+ tiles may fail to autoplay). |
| `TWITCH_FILTER_ADS` | No | Filter embedded Twitch ad segments where supported (default **enabled**; set **`false`** to disable). **`proxy`**: when a media playlist carries stitched-ad markers, the server tries alternate player identities for an ad-free playlist (video keeps playing). **`hls`**: Streamlink reads the stream itself and its Twitch plugin skips embedded ad segments automatically — output pauses during the break, then resumes. **`iframe`**: not available — the official embed is Twitch-controlled, so ad delivery is up to Twitch. |
...
Never commit `.env` or share your client secret. `.gitignore` excludes `.env`, `.sessions/` (legacy), `.sessions.sqlite`, and `.hls-transcode/` (transcoded HLS cache).
...
- **Twitch login** and **Import follows**: OAuth; session is stored in an **HTTP-only cookie** with **session data in SQLite** (`connect-sqlite3`) so login survives **Node process restarts** (not only browser refresh). On **Windows**, the default DB is **`%LOCALAPPDATA%\twitchviewer\sessions.sqlite`** unless you set **`SESSION_FILE_PATH`** (see table above).
...
- **Twitch (default `proxy` mode)**: the server resolves a CDN URL per channel/quality via Twitch's GQL + usher API **directly** (no Streamlink needed — it's used as a fallback if installed), rewrites the playlist, and pipes `.ts` segments through **unchanged** — **no ffmpeg, no transcode, no install**. Each tile picks its quality from its pixel size (small → 360p/480p, large → 720p60), so the browser only decodes what each tile needs. This is what makes 15+ simultaneous streams viable. The legacy **`TWITCH_PLAYBACK=hls`** mode instead runs **streamlink → ffmpeg → H.264/AAC** per live tile and can **max out CPU**; tune **`TWITCH_FFMPEG_*`**, **`TWITCH_STREAMLINK_QUALITY`**, and **`TWITCH_HLS_TIME`** in `.env`, or switch to **`TWITCH_PLAYBACK=iframe`** to avoid local transcoding.

### Notes

- **Twitch video**: the default **`proxy`** mode plays **same-origin HLS** (`/api/twitch-live/…`) via **hls.js** — the server resolves CDN URLs via Twitch's GQL + usher API **directly** (no Streamlink or ffmpeg needed) and proxies Twitch's already-H.264/AAC stream without transcoding. Quality adapts to each tile's size for reliable **muted autoplay** in a multi-cell grid. If **Streamlink** is installed, it's used as a fallback when the direct API fails. The legacy **`hls`** mode additionally requires **ffmpeg** and re-encodes per tile. Otherwise the app uses the official [embed script](https://dev.twitch.tv/docs/embed/video-and-clips/) (`player.twitch.tv/js/embed/v1.js`) and **`Twitch.Player`** with `channel`, `parent`, **`muted: true`**, **`autoplay: true`** — note browsers cap ~16 simultaneous WebGL contexts, so 15+ iframe tiles may never autoplay. Iframe players are created **serially** (short gap between inits) to limit **429** errors. **hls.js** is served from this app (`/hls.min.js`) so strict tracking prevention (e.g. Edge) does not block the script. On Windows, **`install-streamlink.bat`** can install Streamlink for the current user if you do not use pip globally. Console lines mentioning **`chrome-extension://`**, **`background.js`**, or **disconnected port** are almost always **browser extensions**, not this app.
...
- **Twitch (default `proxy` mode)**: the server resolves a CDN URL per channel/quality via Twitch's GQL + usher API **directly** (no Streamlink needed); the server rewrites the playlist and pipes `.ts` segments through unchanged at **`/api/twitch-live/<login>/playlist.m3u8?q=<quality>`** (no ffmpeg). The client picks `?q=` from each tile's pixel size. If the playlist fails to load, check the **server console** (offline channel, or Twitch API change). If **Streamlink** is installed, it's used as a fallback when the direct API fails. See **`TWITCH_STREAMLINK_QUALITY`** and fallbacks in the env table.
- **Twitch (legacy `hls` mode)**: **Streamlink** resolves a short-lived CDN URL; **ffmpeg** re-segments it to **`/api/twitch-live/<login>/playlist.m3u8`**. Requires **both** tools on the server host and runs one ffmpeg transcode per live tile (high CPU). Set **`TWITCH_PLAYBACK=hls`** to opt in.
...
| `.sessions.sqlite` | Local SQLite session DB when not using the Windows default path (see **`SESSION_FILE_PATH`**) |
...
- **Logged out every server restart**: Set **`SESSION_SECRET`** in `.env`; ensure the server can create/write the SQLite session file (see **`SESSION_FILE_PATH`** — on Windows the default is **`%LOCALAPPDATA%\twitchviewer\sessions.sqlite`**).
- **Twitch: “could not load playlist” / spinner forever**: The browser did not get a valid **`playlist.m3u8`**. Confirm the channel is **live**. The default **`proxy`** mode uses Twitch's API directly (no install needed) — check the **server console** for errors (e.g. "channel is offline", GQL errors). If you've installed **Streamlink**, it's used as a fallback automatically. In legacy **`hls`** mode, also ensure **`streamlink --stream-url https://www.twitch.tv/<login> best`** works and **`ffmpeg -version`** succeeds. For a quick workaround, set **`TWITCH_PLAYBACK=iframe`** in `.env` and restart (official embed, no local processing).
- **Twitch tiles never autoplay (15+ streams, iframe mode)**: Browsers cap ~16 simultaneous **WebGL contexts**, and each `Twitch.Player` iframe claims one — past the cap, new tiles initialise but never play. This is a hard limit of the iframe approach. Install **Streamlink** and use the default **`proxy`** mode (same-origin HLS, no WebGL cap) instead.
- **Twitch iframe tiles stuck on a Play button / not autoplaying**: Open the browser console (**F12 → Console**) and run **`twitchviewerIframeDiagnostics()`** — it prints a read-only per-tile report (muted/paused state, `allow="autoplay"`, READY/ONLINE/PLAYING events seen, retry count, PLAYBACK_BLOCKED count, visibility, user activation) that is safe to copy and paste into a bug report. It never prints tokens or cookies. In **Microsoft Edge**, also check **Settings → Cookies and site permissions → Media autoplay** — the site needs autoplay to be at least "Limit" (muted autoplay) rather than "Block".
- **Twitch stutters on fast internet**: In legacy **`hls`** mode this is usually **CPU** (one **ffmpeg** re-encode per live tile), not bandwidth. Switch to the default **`proxy`** mode (no transcode), or lower load with **`TWITCH_STREAMLINK_QUALITY`**, **`TWITCH_FFMPEG_MAX_HEIGHT`**, **`TWITCH_FFMPEG_PRESET`**.
